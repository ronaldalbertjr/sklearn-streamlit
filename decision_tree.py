...
def select_criterion():
    criterion = st.sidebar.selectbox("Escolha o critério de construção da Árvore de Decisão:", 
                                    dataframe.criterions
                                    )
    return criterion

def get_model(kernel):
    model = dataframe.tree[kernel]

    return model

def get_confusion_matrix(kernel):
    confusion_matrix = dataframe.tree_matrix[kernel]
    
    return confusion_matrix

def get_target(kernel):
    iris_target = dataframe.tree_target[kernel]
...
def app():
    ...
    iris_target = get_target(criterion)

    model = get_model(criterion)
    ...
    st.write('Matriz de Confusão:', get_confusion_matrix(criterion))